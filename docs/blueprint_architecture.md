...
## 디렉터리 구조
- `resources/cell_defs.gd`
  - 셀 1개(4바이트) 레이아웃과 플래그 비트, 읽기/쓰기 헬퍼.
- `resources/material_data.gd`
  - 소재 Resource 정의 + 런타임 ID 레지스트리.
- `resources/blueprint.gd`
  - 설계 원본 Resource (`cell_data`, `module_map`, 직렬화/역직렬화).
- `resources/blueprint_stats.gd`
  - bake 시점 자동 산출 스탯 캐시.
- `systems/ship_instance.gd`
  - 런타임 경량 인스턴스(`RefCounted`)와 피격/물리 처리.
- `editor/blueprint_editor.gd`
  - 설계 데이터 조작 + 미리보기 텍스처 갱신.
- `shaders/cell_damage.glsl`
...
## 다음 단계
- `editor/blueprint_editor.gd`에서 데이터 로직과 렌더링 로직 분리.
- `shaders/cell_damage.glsl`를 `ShipInstance.apply_hit()` 경로에 연결.
